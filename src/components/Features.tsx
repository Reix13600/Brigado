  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans">
      ...
      <div className="max-w-4xl mx-auto px-6 pt-14 pb-10 text-center">
        <a href="/" className="inline-flex items-center gap-2 text-xs text-slate-500 hover:text-lime-400 transition-colors mb-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {cat.items.map((item, j) => (
                  <div key={j} className="flex gap-2.5">
                    <Check size={15} className="text-lime-400 flex-shrink-0 mt-0.5" />
                    <div>
                      <h3 className="text-sm font-semibold text-slate-200">{lang === "fr" ? item.titleFr : item.titleEn}</h3>
                      <p className="text-xs text-slate-500 mt-0.5 leading-relaxed">{lang === "fr" ? item.descFr : item.descEn}</p>
                    </div>
                  </div>
                ))}
              </div>
